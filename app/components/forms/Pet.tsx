...
export default function Pet({ onPetChange, rollbackStep, petForm }: { onPetChange: (pet: string) => void; rollbackStep: () => void; petForm: string }) {
    const pets = [
        {
            label: "Oui",
        },
    ...
    const [pet, setPet] = useState<string>(petForm);
    ...
    useEffect(() => {
        setPosition(pets.find((pet) => pet.label === petForm) ? pets.findIndex((pet) => pet.label === petForm) : null);
    }, [petForm]);

    return (
        <div className="space-y-4 lg:space-y-6">
            <div className="flex flex-col gap-4">
                <h2 className="font-bold text-lg lg:text-3xl text-slate-800">Avez-vous un animal de compagnie ?</h2>
                <div className="grid grid-cols-1 gap-3 lg:gap-2.5">
                    {pets.map((pet, index) => (
                        <div key={index} className={`relative flex-1 flex items-center gap-2 p-4 lg:py-4.5 rounded-lg border cursor-pointer ${position === index ? "border-[#f15e00] bg-[#f15e00]/5" : "bg-white border-slate-200"}`} onClick={() => handleSelect(index, pet.label)}>
                            ...
                            <p className="text-center text-slate-700 text-xs lg:text-sm font-semibold">{pet.label}</p>
                        </div>
                    ))}
                </div>
            </div>
